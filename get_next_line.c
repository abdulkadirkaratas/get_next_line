...
/*   Updated: 2024/01/04 20:42:00 by akaratas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_and_join(int fd, char *buffer_set)
{
	...
	while (read_len > 0)
	{
		...
		buffer_set = ft_strjoin(buffer_set, buffer_reader);
...
char	*single_line(char *buffer_set)
{
	...
	if (line_len == -1)
		line_len = ft_strlen(buffer_set);
	else
		line_len = line_len + 1;
	line = ft_substr(buffer_set, 0, line_len);
	return (line);
}
...
char	*update_buffer_set(char *buffer_set)
{
	...
	new_len = ft_strlen(buffer_set) - (nl_index + 1);
	new_buffer_set = ft_substr(buffer_set, (nl_index + 1), new_len);
	free(buffer_set);
